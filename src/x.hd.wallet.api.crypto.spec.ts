...
// __dirname is not needed in Jest; use relative paths from project root
...
describe("Contextual Derivation & Signing", () => {
  ...
  beforeAll(() => {
    rootKey = fromSeed(
      Buffer.from(bip39.mnemonicToSeedSync(bip39Mnemonic, "")),
    );
  });
  ...
  describe("\(JS Library) Reference Implementation alignment with known BIP32-Ed25519 JS LIB", () => {
    it("\(OK) BIP32-Ed25519 derive key m'/44'/283'/0'/0/0", async () => {
      ...
      const derivedPub: Uint8Array =
        crypto_scalarmult_ed25519_base_noclamp(scalar); // calculate public key
  ...
  it("\(OK) Root Key", async () => {
    ...
    expect(Buffer.from(rootKey)).toEqual(
      Buffer.from(
  ...
  describe("\(Derivations) Context", () => {
    ...
    describe("Signing Typed Data", () => {
      it("\(OK) Sign authentication challenge of 32 bytes, encoded base64", async () => {
        ...
        const authSchema: JSONSchemaType<any> = JSON.parse(
          readFileSync(path.resolve("./schemas/auth.request.json"), "utf8"),
        );
        const metadata: SignMetadata = {
          encoding: Encoding.BASE64,
          schema: authSchema,
        };
        const base64Challenge: string =
          Buffer.from(challenge).toString("base64");

        const encoded: Uint8Array = new Uint8Array(
          Buffer.from(base64Challenge),
        );
    ...
    describe("signing transactions", () => {
      it("\(OK) Sign Transaction", async () => {
        ...
        expect(encodeAddress(Buffer.from(key))).toEqual(
    ...
    describe("ECDH cases", () => {
      ...
      it("\(OK) ECDH, Encrypt and Decrypt", async () => {
        ...
        const nonce: Uint8Array = new Uint8Array([
          16, 197, 142, 8, 174, 91, 118, 244, 202, 136, 43, 200, 97, 242, 104,
          99, 42, 154, 191, 32, 67, 30, 6, 123,
        ]);
        ...
        expect(cipherText).toEqual(
          new Uint8Array([
            20, 107, 126, 154, 152, 197, 252, 227, 148, 39, 245, 136, 233, 10,
            199, 20, 219, 3, 53, 2, 113, 6, 190, 21, 193, 119, 43, 44, 230,
          ]),
        );
      ...
      it("Libsodium example ECDH", async () => {
        ...
        const bobXPvt: Uint8Array =
          crypto_sign_ed25519_sk_to_curve25519(bobPvtKey);
        const bobXPub: Uint8Array =
          crypto_sign_ed25519_pk_to_curve25519(bobPubKey);
  ...
  describe("\(deriveNodePrivate)", () => {
    it("\(FAIL) Should fail if during derivation scalar >= 2^255", async () => {
      ...
      expect(deriveChildNodePrivate(derivationNode, 0, g)).rejects.toThrow(
